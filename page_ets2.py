"""
page_ets2.py — Pagina standalone ricerca bandi Terzo Settore
Route: /ets2
"""
from fastapi import APIRouter, Query, Cookie
from fastapi.responses import HTMLResponse, JSONResponse
import db_ets2 as DB, scraper_ets2 as SC
import threading
...
.header{background:linear-gradient(135deg,#1A2A4A,#243555);padding:28px 40px;border-bottom:3px solid #059669}
...
.btn{padding:9px 24px;background:#059669;color:#fff;border:none;border-radius:6px;font-size:0.9rem;font-weight:700;cursor:pointer}
...
.card:hover{border-left-color:#059669;box-shadow:0 3px 12px rgba(26,42,74,0.12)}
.card-top{display:flex;justify-content:space-between;align-items:center;margin-bottom:6px}
.card-tag{font-size:10px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#059669}
...
.btn-fonte{padding:6px 12px;background:none;color:#059669;border:1px solid #D0DCF0;border-radius:5px;font-size:0.73rem;font-weight:700;cursor:pointer;text-decoration:none}
.btn-fonte:hover{border-color:#059669}
...
<title>ItalBandi ETS — Bandi per il Terzo Settore</title>{CSS}
</head><body>
<div class="header">
  <h1>🤝 Bandi <span>Terzo Settore</span></h1>
  <p>Finanziamenti per ETS, ODV, APS, cooperative sociali, fondazioni e ONLUS</p>
...
<footer>ItalBandi ETS — un servizio di <strong>Energelia S.r.l.</strong> · 010 8078800</footer>
<script>
async function cerca() {{
  ...
  const r = await fetch('/api/ets2?' + p);
  ...
  document.getElementById('ris').innerHTML = d.bandi.map(b => {{
    ...
    return `<div class="card">
      <div class="card-top">
        <span class="card-tag">🤝 Terzo Settore</span>
...
@router.get("/ets2", response_class=HTMLResponse)
async def ets2_page(session_id: str = Cookie(default=None)):
...
@router.post("/api/ets2/scrapa")
async def ets2_scrapa(session_id: str = Cookie(default=None)):
    from main import get_session
    user = get_session(session_id)
    if not user or not user.get("is_admin"):
        return JSONResponse({"error": "Non autorizzato"}, status_code=403)
    threading.Thread(target=SC.scrapa, daemon=True).start()
    return JSONResponse({"ok": True, "messaggio": "Scraper ETS2 avviato"})
